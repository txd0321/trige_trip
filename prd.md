...
3. **OpenCV 云函数识别**
   - 客户端裁剪 ROI 图像并上传云函数（建议 480px 高度压缩）。
   - 云函数使用 OpenCV 识别 5 段的圆形 / 叉 / 空（轮廓 + 形状匹配）。
   - 返回 5 段识别结果、置信度与耗时。
4. **可视化反馈**
   - 叠加透明 Canvas 层，实时绘制 5 段矩形框。
   - 识别结果高亮：绿色-圆形，橙色-叉，半透明白-空。
   - 顶部实时显示形状组合，如 `● ❌ — ❌ —`。
5. **音符判定与反馈**
   - 从上到下找到第一个圆形段映射为音符（如 F5/E5）。
   - 识别成功时 Toast 提示音符名称，并播放音效（当前为震动代替）。
6. **进度累计与完成**
   - 按顺序完成 13 次目标模式识别（有重复音符）。
   - 仅当当前帧模式与“当前目标模式”一致时才计为成功。
   - 成功判定后等待 2 秒进入下一任务。
...
2. 发条轮廓扫描：OpenCV 云函数模板/轮廓匹配，返回对齐分数与角度。
...
- **OpenCV 云函数识别**：`POST /api/vision/winder`
  - 请求：`{ imageBase64, roi: { x,y,width,height }, seq, timestamp }`
...
- **错误码与埋点建议**：
  ...
  - `WINDER_ABORT`：用户中途退出
  - `WINDER_NET_ERROR`：云函数请求失败
  - 关键埋点：进入步骤、对齐成功、开始旋转、完成旋转、超时失败、网络异常
- **异常处理**：
  - 云函数返回失败时，展示“网络不稳定，请稍后再试”。
