...
用户故事 / 场景  
- 作为用户，我想要快速登录，并自动使用我的微信昵称和头像，只需点击一次即可完成。

功能点  
1. 自动获取并展示微信头像 & 昵称（`wx.getUserProfile`）  
2. 用户协议 & 隐私协议勾选校验，未勾选禁止点击登录  
3. 「微信授权登录」按钮  
   - 调用 `wx.login` 获取 `code`  
...
## 页面E. AR 探索扫描页
用户故事 / 场景  
-作为用户，我可以使用摄像头依次对准实体产品上的特定图案代表的不同音符，在手机屏幕上得到不同音调的反馈，我可以看到扫描的正确与否以及扫描的进度，一共有13个音符，扫描正确90%以上则可以跳转到下一个任务：发条解锁与旋转页面。在这个页面中我可以后退返回到主页。
