...
3. **OpenCV 云托管识别（推荐）**
   ...
   - 云托管使用 OpenCV 识别 5 段的 circle / cross / empty（自适应二值化 + 形态学 + 模板比对）。
   - 返回 5 段识别结果、置信度与耗时。
4. **可视化反馈**
...
- **云托管处理流程（ROI 识别）**
  ...
  4. 统一等分 5 段 → 自适应二值化 → 形态学开闭运算。
  5. 每段做模板比对（circle/cross）+ 像素占比与轮廓兜底（抗距离缩放）。
  6. 输出 slots 与 confidence（包含稳定度），返回耗时。
